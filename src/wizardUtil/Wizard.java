...
public class Wizard extends Panel implements ActionListener {
	...
	*/
    ...
    private void updateButtons() {
        cancelButton.setDisabled(true);
        helpButton.setDisabled(!current.hasHelp());
        backButton.setDisabled(!(previous.size() > 0));
        nextButton.setDisabled(!current.hasNext());
        finishButton.setDisabled(!current.canFinish());
    }
    ...
    private void showErrorMessages(ArrayList list) {
        ...
        Clients.showNotification(""+list, true);
